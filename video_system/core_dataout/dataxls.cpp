    ...
    for (int i = 0; i < rowCount; i++) {
        ...
        if (dataContent.randomColor) {
            ...
            for (int j = 0; j < columnCount; j++) {
                if (dataContent.colorColumn.size() == 0) {
...
{
    //行数不对则返回
    int size = dataContent.content.size();
    if (size <= 0) {
        return;
    }
    ...
    int contentCount = listContent.size();
    int subTitle1Count = listSubTitle1.size();
    int subTitle2Count = listSubTitle2.size();
    ...
    emit appendFinshed(size, time.elapsed());
}
...
void DataXls::open()
{
    columnCount = dataContent.columnNames.size();
    ...
    //输出字段名称
    if (dataContent.subTitle1.size() == 0) {
        appendColumn();
    }
}
