...
bool DataHelper::checkDataContent(DataContent &dataContent)
{
    ...
    int columnCount = dataContent.columnNames.size();
    int widthCount = dataContent.columnWidths.size();
    ...
    //没有设置的对应列需要设置下
    //防止没有对应的对齐值下面取值会溢出
    int size1 = dataContent.columnAlignment.size();
    int size2 = dataContent.contentAlignment.size();
