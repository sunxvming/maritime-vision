...
private:
    ...
    QTimer *timerRestart;
    //检测覆盖层清理定时器
    QTimer *timerClearDetection;
    //记录每个视频控件最后一次接收检测数据的时间戳
    QMap<VideoWidget*, qint64> lastDetectionTime;
    //通道布局盒子
    VideoBox *videoBox;
...
private slots:
    ...
    void onAlarmEventReceived(const AlarmEvent &result);
