...
void frmVideoPlayImage::on_btnSelect_clicked()
{
    ...
    while (dateStart <= dateEnd) {
        ...
        if (saveDir.exists()) {
            ...
            if (size > 0) {
                QString text = QString("%1 %2 %3张").arg(dateName).arg(channel).arg(size);
...
void frmVideoPlayImage::on_btnOpenDir_clicked()
{
    // 打开保存的文件夹，而不是下载
    QString filePath = AppData::ImageNormalPath;
...
void frmVideoPlayImage::on_btnDelete_clicked()
{
    ...
    QUIHelper::showMessageBoxInfo("删除图片文件完成!", 3);
    //立即重新查询
    on_btnSelect_clicked();
}
...
void frmVideoPlayImage::on_btnClear_clicked()
{
    ...
    QUIHelper::showMessageBoxInfo("清空图片文件完成!", 3);
    //立即重新查询
    on_btnSelect_clicked();
}
...
void frmVideoPlayImage::on_listWidget_doubleClicked()
{
    ...
    if (size > 0) {
        listIndex = 0;
        ui->labDuration->setText(QString("共 %1 张").arg(size));
        ui->sliderPosition->setRange(0, size - 1);
        ui->sliderPosition->setValue(0);
