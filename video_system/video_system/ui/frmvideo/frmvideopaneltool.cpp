...
void frmVideoPanelTool::stopAlarm()
{
    //停止报警闪烁定时器并恢复按钮的样式
    timerAlarm->stop();
    QString qss;// = QString("QPushButton{color:%1;}").arg(QUIConfig::TextColor);
    qss += QString("QPushButton:hover{color:%1;}").arg(QUIConfig::DarkColorStart);
    qss += QString("QPushButton:pressed{color:%1;}").arg(QUIConfig::HighColor);
...
void frmVideoPanelTool::doAlarm()
{
    ...
    QString color = isAlarm ? QUIConfig::TextColor : AppConfig::ColorIconAlarm;
