...
void frmIpcPolling::setText()
{
    //统计轮询信息,如果不需要可以不显示
    int size = pollUrls.size();
    QString info = QString("第 %1 次 / 第 %2 组 / 共 %3 组").arg(pollCount).arg(pollIndex).arg(size);
...
void frmIpcPolling::getPollUrls()
{
    ...
    for (int i = 0; i < DbData::PollInfo_Count; ++i) {
        ...
        if (list.size() == AppConfig::PollType) {
            pollUrls << list.join(";");
            list.clear();
        }
    }
...
void frmIpcPolling::on_btnPollStart_clicked()
{
    ...
    if (AppData::videoPanel->actionPoll->text() == "启动轮询视频") {
        getPollUrls();
        if (pollUrls.size() == 0) {
            QUIHelper::showMessageBoxError("当前分组没有视频需要轮询, 请在轮询配置中添加!", 3);
            return;
        }
