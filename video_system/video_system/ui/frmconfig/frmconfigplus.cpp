...
void frmConfigPlus::initConfig()
{
    ...
    listNumber << "2" << "5" << "10" << "15" << "20" << "30" << "50" << "100" << "250";
...
void frmConfigPlus::on_btnAddPlus_clicked()
{
    ...
    for (int i = 0; i < AppConfig::PlusNumber; ++i) {
        ...
        if (QUIHelper::isIP(AppConfig::PlusAddr)) {
...
void frmConfigPlus::on_cboxPlusType_currentIndexChanged(int index)
{
    ...
    if (index == 0) {
        rtspMain = "f:/mp4/[Addr].mp4";
        rtspSub = "f:/mp4/[Addr].mp4";
    } else if (index >= 1 && index <= 4) {
        DeviceType deviceType = (DeviceType)index;
        rtspMain = UrlHelper::getRtspUrl(deviceType, 0, 0);
        rtspSub = UrlHelper::getRtspUrl(deviceType, 0, 1);
    } else {
        rtspMain = UrlHelper::getRtspUrl(DeviceType_Normal, 0, 0);
        rtspSub = UrlHelper::getRtspUrl(DeviceType_Normal, 0, 1);
    }
