...
private:
    ...
    // 场景数据缓存
    QMap<int, QString> m_sceneMap;  // scene_id -> scene_name
    ...
    void loadScenes();
    void loadIpcList();
    void onScenesLoaded(const QJsonObject &data);
    void onIpcDataLoaded(const QJsonArray &list);
    void onAllSavesCompleted(bool hasError);
...
private slots:
    ...
    void onAlgorithmConfigClicked(int row);
};
