...
{
    if (row < 0) {
        QUIHelper::showMessageBoxError("请先选择要删除的分组!", 3);
        return false;
    } else if (row == 0) {
        QUIHelper::showMessageBoxError("默认分组不能删除!", 3);
        return false;
    }
...
bool frmConfigPollPlus::checkPollGroup(const QString &pollGroup)
{
    if (pollGroup.isEmpty()) {
        QUIHelper::showMessageBoxError("请先输入分组名称!", 3);
    ...
    for (int i = 0; i < count; ++i) {
        if (ui->listWidget->item(i)->text() == pollGroup) {
            QUIHelper::showMessageBoxError("该分组已经存在, 请重新输入!", 3);
            ui->txtPollGroup->setFocus();
            return false;
        }
    }
...
void frmConfigPollPlus::on_listWidget_currentItemChanged(QListWidgetItem *current, QListWidgetItem *previous)
{
    //需要加个判断
    if (current) {
        ui->txtPollGroup->setText(current->text());
    }
}
