...
void frmConfigSystem::initVideoConfig2()
{
    ...
    VideoHelper::loadHardware(ui->cboxHardware, VideoCore_FFmpeg, AppConfig::Hardware);
    ...
    VideoHelper::loadCaching(ui->cboxCaching);
    ui->cboxCaching->setCurrentIndex(ui->cboxCaching->findData(QString::number(AppConfig::Caching)));
    connect(ui->cboxCaching, SIGNAL(currentIndexChanged(int)), this, SLOT(saveVideoConfig2()));

    VideoHelper::loadReadTimeout(ui->cboxReadTimeout);
    ...
    VideoHelper::loadConnectTimeout(ui->cboxConnectTimeout);
...
void frmConfigSystem::initEnableConfig1()
{
    ...
    //懒得一个个写直接循环遍历就行
    QList<QCheckBox *> ckboxs = ui->frameEnable1->findChildren<QCheckBox *>();
