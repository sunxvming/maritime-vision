...
void frmConfigIpcSearch::initData()
{
    ...
    QUIHelper::initTableView(ui->tableWidget, AppData::RowHeight, true, false);
...
void frmConfigIpcSearch::initConfig()
{
    ...
    QStringList searchInterval;
    searchInterval << "1" << "2" << "3" << "4" << "100" << "300" << "500" << "1000" << "2000" << "3000" << "5000";
    ui->cboxSearchInterval->addItems(searchInterval);
    ui->cboxSearchInterval->setCurrentIndex(ui->cboxSearchInterval->findText(QString::number(AppConfig::SearchInterval)));
    connect(ui->cboxSearchInterval, SIGNAL(activated(int)), this, SLOT(saveConfig()));

    QStringList searchTimeout;
    searchTimeout << "1000" << "1500" << "2000" << "3000" << "5000" << "8000";
    ui->cboxSearchTimeout->addItems(searchTimeout);
    ui->cboxSearchTimeout->setCurrentIndex(ui->cboxSearchTimeout->findText(QString::number(AppConfig::SearchTimeout)));
...
void frmConfigIpcSearch::saveConfig()
{
    ...
    AppConfig::SearchInterval = ui->cboxSearchInterval->currentText().toInt();
    AppConfig::SearchTimeout = ui->cboxSearchTimeout->currentText().toInt();
    OnvifRequest::timeout = AppConfig::SearchTimeout;
...
void frmConfigIpcSearch::receiveDevice(const OnvifDeviceInfo &deviceInfo)
{
    ...
    //取IP地址转成整型作为排序依据
    quint32 ipAddr = QUIHelper::ipv4StringToInt(ip);
    ...
    ui->labCount->setText(QString("共搜索到 %1 个设备").arg(devices.size()));
}
...
void frmConfigIpcSearch::disableRow(const QString &addr, int row)
{
    ...
    QColor textColor = QUIConfig::TextColor;
    QColor bgColor = QUIConfig::DarkColorEnd;
...
void frmConfigIpcSearch::getMedia(int row, OnvifDevice *device)
{
    ...
    int countProfile = profiles.size();
    if (countProfile == 0) {
        QUIHelper::showMessageBoxError("当前设备没有配置文件, 请仔细检查!", 3);
        return;
    }

    QString profileToken, videoSource, rtspMain, rtspSub;
    //如果有第二个则为子码流
    if (countProfile > 1) {
    ...
    videoSource = videoSources.first().token;

#if 1
    //如果有多个视频源则一般是NVR(也有部分相机同时带两路视频流)
    int countVideo = videoSources.size();
    if (countVideo > 1) {
        ...
        if (company == "DAHUA") {
            for (int i = 0; i < countProfile; i = i + 3) {
...
void frmConfigIpcSearch::addDevice(int row, bool one)
{
    ...
    int size = listProfileToken.size();
    for (int i = 0; i < size; ++i) {
...
void frmConfigIpcSearch::on_btnAddOne_clicked()
{
    int row = ui->tableWidget->currentRow();
    if (row < 0) {
        QUIHelper::showMessageBoxError("请先选中要添加的设备!", 3);
        return;
    }
