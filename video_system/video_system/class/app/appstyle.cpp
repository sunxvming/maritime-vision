...
void AppStyle::addCustomStyle(QStringList &list)
{
    ...
    list << QString("QWidget#frmVideoPanelTool>QPushButton{color:%1;}").arg(QUIConfig::TextColor);
    ...
    //标签加粗居中背景色突出等样式
    list << QString("QLabel#labTip{font-weight:bold;font-size:%1px;min-height:%2px;}").arg(QUIConfig::FontSize + 2).arg(28);
    list << QString("QLabel#labTip{background-color:%1;qproperty-alignment:AlignCenter;}").arg(QUIConfig::NormalColorEnd);
    ...
    //播放按钮
    list << QString("#frameSlider>QPushButton{background:none;border:none;padding:0px 0px 0px 0px;}");
    list << QString("#frameSlider>QPushButton{color:%1;}").arg(QUIConfig::TextColor);
    //list << QString("#frameSlider>QPushButton:hover{color:%1;}").arg(QUIConfig::DarkColorEnd);

    //录像计划任务策略表格
    list << QString("TaskTableView{qproperty-bgColor:%1;}").arg(QUIConfig::PanelColor);
    list << QString("TaskTableView{qproperty-gridColor:%1;}").arg(QUIConfig::BorderColor);
    list << QString("TaskTableView{qproperty-headerBgColor:%1;}").arg(QUIConfig::NormalColorStart);
    list << QString("TaskTableView{qproperty-headerTextColor:%1;}").arg(QUIConfig::TextColor);
    list << QString("TaskTableView{qproperty-normalColor:%1;}").arg(QUIConfig::NormalColorStart);
    list << QString("TaskTableView{qproperty-selectColor:%1;}").arg(QUIConfig::HighColor);
    list << QString("TaskTableView{qproperty-shadowWidth:%1;}").arg(0);
    list << QString("TaskTableView{qproperty-shadowColor:%1;}").arg(QUIConfig::DarkColorStart);

    //音量条控件
    list << QString("BarVolume{qproperty-radius:%1;}").arg(0);
    list << QString("BarVolume{qproperty-space:%1;}").arg(1);
    list << QString("BarVolume{qproperty-barBgColor:%1;}").arg(QUIConfig::TextColor);
    list << QString("BarVolume{qproperty-bgColorStart:%1;}").arg(QUIConfig::NormalColorStart);
    list << QString("BarVolume{qproperty-bgColorEnd:%1;}").arg(QUIConfig::NormalColorEnd);
}
