...
{
    return verticalLabel.size() + 1;
}

int TaskModel::columnCount(const QModelIndex &) const
{
    return horizontalLabel.size() + 1;
}
...
QVariant TaskModel::data(const QModelIndex &index, int role) const
{
    ...
    if (role == Qt::ToolTipRole) {
        if (row > 0 && column > 0) {
            //return verticalLabel.at(row - 1) + " " + horizontalLabel.at(column - 1);
        }
    } else if (role == Qt::TextAlignmentRole) {
        return QVariant(Qt::AlignLeft | Qt::AlignVCenter);
    } else if (role == Qt::UserRole) {
        if (column == 0 && row > 0 && row <= verticalLabel.size()) {
            return verticalLabel.at(row - 1);
        } else if (row == 0 && column > 0 && column <= horizontalLabel.size()) {
            return horizontalLabel.at(column - 1);
        }
    }
...
void TaskTableView::initForm()
{
    ...
    //行数列数
    rowCount = verticalLabel.size();
    columnCount = horizontalLabel.size();
