...
{
    addUserLog("0", logType, logContent, "");
}
...
void DbQuery::loadIpcInfo(std::function<void()> onDone)
{
    ApiClient::instance()->getAllIpc(
        [onDone](const QJsonArray &ipcList) {
            ...
            for (const QJsonValue &val : ipcList) {
                QJsonObject obj = val.toObject();

                int ipcID = obj.value("ipc_id").toInt();
                QString ipcName = obj.value("ipc_name").toString();
                QString nvrName = obj.value("nvr_name").toString();
                QString ipcType = obj.value("ipc_type").toString();
                QString onvifAddr = obj.value("onvif_addr").toString();
                QString profileToken = obj.value("profile_token").toString();
                QString videoSource = obj.value("video_source").toString();
                QString rtspMain = obj.value("rtsp_main").toString();
                QString rtspSub = obj.value("rtsp_sub").toString();
                QString ipcPosition = obj.value("ipc_position").toString();
                QString ipcImage = obj.value("ipc_image").toString();
                int ipcX = obj.value("ipc_x").toInt();
                int ipcY = obj.value("ipc_y").toInt();
                QString userName = obj.value("user_name").toString();
                QString userPwd = obj.value("user_pwd").toString();
                QString ipcEnable = obj.value("ipc_enable").toString();
            ...
            if (AppData::MapNames.size() > 0 && !AppData::MapNames.contains(AppData::CurrentImage)) {
                AppData::CurrentImage = AppData::MapNames.at(0);
            }

            if (onDone) onDone();
        },
        [onDone](const QString &error) {
            qDebug() << TIMEMS << "loadIpcInfo: HTTP request failed:" << error;
            if (onDone) onDone();
        }
    );
}
...
void DbQuery::deleteIpcInfos(const QString &ids, std::function<void()> onDone)
{
    ...
    state->total = idList.size();
    state->completed = 0;
    state->callback = onDone;

    for (const QString &idStr : idList) {
        int ipcId = idStr.trimmed().toInt();
        if (ipcId <= 0) {
            state->completed++;
            if (state->completed >= state->total && state->callback) {
                state->callback();
            }
            continue;
        }

        ApiClient::instance()->deleteIpc(ipcId,
            ...
            [state, ipcId](const QString &error) {
                qDebug() << TIMEMS << "deleteIpcInfos failed for id" << ipcId << ":" << error;
                state->completed++;
                if (state->completed >= state->total && state->callback) {
                    state->callback();
                }
...
void DbQuery::updatePosition(DeviceButton *btn)
{
    ...
    ApiClient::instance()->updateIpcPosition(ipcId, data,
        [](const QJsonObject &) {
            // Success - no action needed
        },
        [](const QString &error) {
            qDebug() << TIMEMS << "updatePosition(btn) failed:" << error;
        }
    );
}
...
{
    QJsonObject data;
    data.insert("ipc_position", position);

    ApiClient::instance()->updateIpcPosition(id, data,
        [id, position](const QJsonObject &) {
            DbQuery::loadIpcInfo([]() {
                AppEvent::Instance()->slot_saveIpcInfo();
            });
        },
        [](const QString &error) {
            qDebug() << TIMEMS << "updatePosition(id, position) failed:" << error;
        }
    );
}
