...
void DeviceHelper::setTableWidget(QTableWidget *tableWidget)
{
    ...
    QUIHelper::initTableView(deviceTable, AppData::RowHeight, false, false);
}
...
void DeviceHelper::initVideoIcon()
{
    ...
    //可以自行查找对照表改成不同的图标
    QPixmap iconNvr = IconHelper::getPixmap(QUIConfig::TextColor, 0xea23, size, width, height);
    //如果没有启动离线检测则默认在线
    QString color = AppConfig::CheckOffline ? AppConfig::ColorIconAlarm : QUIConfig::TextColor;
...
void DeviceHelper::setVideoIcon2(const QString &ip, bool online)
{
    ...
    //摄像头图标在线离线
    QPixmap iconIpc = IconHelper::getPixmap(QUIConfig::TextColor, 0xea07, size, width, height);
    QPixmap iconSub = IconHelper::getPixmap(QUIConfig::TextColor, 0xe9ff, size, width, height);
...
int DeviceHelper::initUrls()
{
    ...
    if (AppConfig::SaveUrl) {
        QString fileName = QUIHelper::appPath() + "/config/url.txt";
        VideoUrl::readUrls(fileName, AppData::VideoUrls);

        //地址要求在摄像机表中存在并且处于开启状态
        // for (int i = 0; i < AppData::VideoUrls.size(); ++i) {
...
void DeviceHelper::showVideo(const QString &url, int recordTime)
{
    if (url.isEmpty()) {
        return;
    }
    ...
    if (online) {
        frmVideoPreview *video = new frmVideoPreview;
        video->open(url, recordTime);
    } else {
        QUIHelper::showMessageBoxError("设备不在线, 请重新检查!");
    }
}
...
bool DeviceHelper::checkOnline(const QString &url)
{
    ...
    int timeout = QUIHelper::getRandValue(1000, 2000);
    return QUIHelper::ipLive(ip, port, timeout);
}
...
void DeviceHelper::initVideoWidget(VideoWidget *videoWidget, int soundValue, bool soundMuted, bool bannerEnable, int readTimeout)
{
    ...
    widgetPara.bgImage = QImage(QString("%1/config/bg_novideo.png").arg(QUIHelper::appPath()));
    ...
    videoPara.audioLevel = (AppConfig::WorkMode == 3 ? true : false);
