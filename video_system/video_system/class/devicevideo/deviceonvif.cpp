...
QList<OnvifPresetInfo> DeviceOnvif::getPresets(const QString &url)
{
    ...
    if (checkUrl(url, deviceData)) {
        ...
        device->debug("onvif组件", QString("获取预置位置: %1").arg(presets.size()));
    }
    return presets;
}
...
void DeviceOnvif::initData()
{
    ...
    QFile file(QUIHelper::appPath() + "/config/event.txt");
...
void DeviceOnvif::checkOnline()
{
    ...
    if (url.startsWith("rtsp")) {
        ...
        int index = -1;
        int count = AppData::VideoUrls.size();
...
void DeviceOnvif::receiveEvent(const QString &url, const OnvifEventInfo &event)
{
    ...
    if (!eventName.contains(name)) {
        ...
        qDebug() << TIMEMS << "未知报警事件" << QUIHelper::getIP(url) << name << value;
        return;
    } else {
    ...
    if (alarm) {
        ...
        if (videoWidget) {
            ...
            QString fileName = QString("%1/image_alarm/%2/%3_%4.jpg").arg(QUIHelper::appPath()).arg(QDATE).arg(objName).arg(STRDATETIMEMS);
    ...
    if (alarm && info == "开关量报警") {
        ...
        DeviceHelper::showVideo(url, AppConfig::AlarmSaveTime);
    }
}
