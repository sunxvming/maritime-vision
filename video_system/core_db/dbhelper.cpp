...
QStringList DbHelper::getDbType()
{
    static QStringList list;
    if (list.size() == 0) {
        list << "Sqlite" << "MySql" << "PostgreSQL" << "SqlServer" << "Oracle" << "KingBase" << "Other";
    }
...
#endif
            //access数据库连接字符串
            //DRIVER={Microsoft Access Driver (*.mdb, *.accdb)};FIL={MS Access};DBQ=test.mdb
        } else if (dbType == "ORACLE") {
            //貌似只能连接全局数据库名称 orcl
