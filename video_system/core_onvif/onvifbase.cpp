...
bool OnvifBase::getServices()
{
    ...
    if (ok) {
        ...
        OnvifQuery query;
        if (query.setData(dataReceive)) {
            OnvifHttpAddr httpAddr = query.getServices(device->ipport);
...
bool OnvifBase::getCapabilities()
{
    ...
    if (ok) {
        ...
        OnvifQuery query;
        if (query.setData(dataReceive)) {
            OnvifHttpAddr httpAddr = query.getCapabilities(device->ipport);
            device->mediaUrl = httpAddr.addrMedia;
            device->ptzUrl = httpAddr.addrPtz;
        }
    }
...
QString OnvifBase::getStreamUri(const QString &profileToken)
{
    ...
    if (ok) {
        ...
        OnvifQuery query;
        if (query.setData(dataReceive)) {
            device->rtspUrl = query.getStreamUri(device->ipport);
        }
    }
