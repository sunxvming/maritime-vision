...
OnvifDevice::OnvifDevice(QObject *parent) : QObject(parent)
{
    ...
    connect(request, SIGNAL(sendData(QByteArray, QString)), this, SIGNAL(sendData(QByteArray, QString)));
    connect(request, SIGNAL(receiveData(QByteArray)), this, SIGNAL(receiveData(QByteArray)));
...
{
    return OnvifXml::getUserToken(userName, userPwd);
}
...
void OnvifDevice::debug(const QString &head, const QString &msg)
{
    ...
    QString addr = onvifAddr;
    if (debugInfo == 2) {
        //为空的时候获取一次即可
        if (ip.isEmpty()) {
            ip = OnvifHelper::getUrlIP(onvifAddr);
        }
        addr = ip;
    }
...
{
    return onvifOther->setDateTime(datetime, ntp);
}
