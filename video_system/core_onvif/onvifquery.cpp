...
QString OnvifQuery::getValue2(const QString &key)
{
    //直接暴力取节点 比如 wsa5:Address
    QString value = "-1";
    int indexStart = buffer.indexOf(key.toUtf8()) + key.length() + 1;
    int indexEnd = buffer.indexOf(QString("/" + key).toUtf8()) - 1;
    //qDebug() << key << indexStart << indexEnd;
    if (indexStart >= 0 && indexEnd >= 0 && indexStart < indexEnd) {
        value = buffer.mid(indexStart, indexEnd - indexStart);
    }

    return value;
}

QString OnvifQuery::getValue3(const QString &key)
{
    //字符串匹配取节点 比如 Address
    QString value = "-1";
    int indexStart = buffer.indexOf(key.toUtf8()) + key.length() + 1;
    int indexEnd = buffer.lastIndexOf(key.toUtf8());
    //qDebug() << key << indexStart << indexEnd;
    if (indexStart >= 0 && indexEnd >= 0 && indexStart < indexEnd) {
        value = buffer.mid(indexStart, indexEnd - indexStart);
...
QString OnvifQuery::getValue4(const QString &key)
{
    //直接暴力取节点 比如 wsa5:Address 自动拼成<wsa5:Address> 找位置
    QString value = "-1";
    QString key1 = QString("<%1>").arg(key);
    QString key2 = QString("</%1>").arg(key);
    int indexStart = buffer.indexOf(key1.toUtf8()) + key1.length();
    int indexEnd = buffer.indexOf(key2.toUtf8());
    //qDebug() << key << indexStart << indexEnd;
    if (indexStart >= 0 && indexEnd >= 0 && indexStart < indexEnd) {
        value = buffer.mid(indexStart, indexEnd - indexStart);
    }
...
bool OnvifQuery::setData(const QByteArray &data, const QString &fileName)
{
    ...
    for (int i = 0; i < count; ++i) {
        ...
        QStringList list = nodeName.split(':', Qt::SkipEmptyParts);
#else
        QStringList list = nodeName.split(':', QString::SkipEmptyParts);
#endif
        if (list.length() == 2) {
            nodeName = list.at(1);
        }
...
OnvifHttpAddr OnvifQuery::getCapabilities(const QString &ipport)
{
...
QList<OnvifProfileInfo> OnvifQuery::getProfiles()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        int size1 = nodeMin.size();
        int size2 = nodeMax.size();
...
QString OnvifQuery::getStreamUri(const QString &ipport)
{
    QString addr = getValueByTagName("Uri", wsdlAddr.schema);
    this->checkAddr(ipport, addr, false, false);
    return addr;
}

QString OnvifQuery::getSnapshotUri(const QString &ipport)
{
    QString addr = getValueByTagName("Uri", wsdlAddr.schema);
    this->checkAddr(ipport, addr, false, true);
    return addr;
}

QString OnvifQuery::getEventAddr(const QString &ipport)
{
...
OnvifEventInfo OnvifQuery::getEventInfo()
{
    ...
    int size = nodeTime.size();
    int size1 = nodeSource.size();
...
QList<OnvifVideoSource> OnvifQuery::getVideoSources()
{
    ...
    int size = nodeVideoSources.size();
    for (int i = 0; i < size; ++i) {
