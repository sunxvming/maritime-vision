...
struct OsdInfo {
    ...
    bool invertedColors;        //是否反色(背景用文本颜色,文本用白色)
    bool bold;                  //是否粗体
...
struct BannerPara {
    ...
    BannerPara() {
        ...
        //采用iconfont图形字体
        icons = QList<int>() << 0xea1b << 0xeb15 << 0xe674 << 0xe74c;
        //为了避免和其他控件重名建议前面加上前缀用来区分
        names = QList<QString>() << "banner_btnRecord" << "banner_btnSnap" << "banner_btnSound" << "banner_btnClose";
        tips = QList<QString>() << "录制" << "抓图" << "声音" << "关闭";
    }
};
