...
{
    //第四个参数用来标识是否采用本地对话框
    QString fileName = QUIHelper::getOpenFileName(filter, AppConfig::SelectDirName, defaultName);
...
{
    //第四个参数用来标识是否采用本地对话框
    QString fileName = QUIHelper::getSaveFileName(filter, AppConfig::SelectDirName, defaultName);
    saveSelectDirName(fileName);
    return fileName;
}
...
bool FormHelper::inputData(QSqlTableModel *model, const QStringList &columnNames, const QString &tableName, const QString &flag)
{
    ...
    bool ok = DataCsv::inputData(columnNames.size(), "", tableName, fileName, true);
    if (!fileName.isEmpty()) {
...
bool FormHelper::outputData(const QString &tableName, const QString &flag)
{
    ...
    QString name = QString("%1/db/%2%3").arg(QUIHelper::appPath()).arg(flag).arg(DataCsv::CsvExtension);
...
bool FormHelper::checkRowCount(quint8 type, int rowCount)
{
    ...
    return QUIHelper::checkRowCount(rowCount, maxCount, warnCount);
}
