...
QString QUIHelperNet::getIP(const QString &url)
{
    //取出IP地址
...
bool QUIHelperNet::isIP(const QString &ip)
{
    QRegExp regExp("((2[0-4]\\d|25[0-5]|[01]?\\d\\d?)\\.){3}(2[0-4]\\d|25[0-5]|[01]?\\d\\d?)");
    return regExp.exactMatch(ip);
}

bool QUIHelperNet::isMac(const QString &mac)
{
    QRegExp regExp("^[A-F0-9]{2}(-[A-F0-9]{2}){5}$");
    return regExp.exactMatch(mac);
}

bool QUIHelperNet::isTel(const QString &tel)
{
    if (tel.length() != 11) {
        return false;
    }
...
bool QUIHelperNet::isEmail(const QString &email)
{
...
QString QUIHelperNet::ipv4IntToString(quint32 ip)
{
    QString result = QString("%1.%2.%3.%4").arg((ip >> 24) & 0xFF).arg((ip >> 16) & 0xFF).arg((ip >> 8) & 0xFF).arg(ip & 0xFF);
    return result;
}

quint32 QUIHelperNet::ipv4StringToInt(const QString &ip)
{
    int result = 0;
    if (isIP(ip)) {
...
bool QUIHelperNet::ipLive(const QString &ip, int port, int timeout)
{
    //局部的事件循环,不卡主界面
...
bool QUIHelperNet::download(const QString &url, const QString &fileName, int timeout)
{
...
QByteArray QUIHelperNet::getHtml(const QString &url, int timeout)
{
    //初始化网络请求对象
...
QString QUIHelperNet::getNetIP(const QString &html)
{
    QString ip;
...
QString QUIHelperNet::getLocalIP()
{
...
QStringList QUIHelperNet::getLocalIPs()
{
...
QString QUIHelperNet::urlToIP(const QString &url)
{
