...
QPixmap QUIHelperImage::getPixmap(QWidget *widget, const QPixmap &pixmap, bool scale)
{
    if (pixmap.isNull()) {
        return pixmap;
    }
...
void QUIHelperImage::setPixmap(QLabel *label, const QString &file, bool scale)
{
    //文件不存在则不用处理
...
void QUIHelperImage::setLogo(QLabel *label, const QString &file,
                             ...
                             const QString &oldColor, const QString &newColor)
{
    ...
    if (file.startsWith("icon")) {
        ...
        if (!file.startsWith(":/")) {
            height = 50;
            fileName = QString("%1/logo/%2").arg(QUIHelperCore::appPath()).arg(file);
        }
        ...
        if (file.endsWith(".svg")) {
            ...
            if (f.open(QFile::ReadOnly)) {
                ...
                //目录不存在则新建
                QUIHelperCore::checkPath("logo");
                //打开新的文件输出
                fileName = QString("%1/logo/temp.svg").arg(QUIHelperCore::appPath());
                QFile f2(fileName);
...
QRect QUIHelperImage::getCenterRect(const QSize &imageSize, const QRect &widgetRect, int borderWidth, int scaleMode)
{
...
void QUIHelperImage::getScaledImage(QImage &image, const QSize &widgetSize, int scaleMode, bool fast)
{
